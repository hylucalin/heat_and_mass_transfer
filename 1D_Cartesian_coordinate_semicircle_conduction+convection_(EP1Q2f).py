...
def find_unit_circle():
  ...
  Num_of_points_on_unit_circle = len(unit_circle_list)
  # Convert to NumPy array after the loop (faster)
  unit_circle_list = np.array(unit_circle_list)
  print(unit_circle_list)
  return unit_circle_list, Num_of_points_on_unit_circle

unit_circle_list, Num_of_points_on_unit_circle = find_unit_circle()
arc_length = np.pi / (2*Num_of_points_on_unit_circle)
...
#  print(f'At time {time} sec.')
  ...
  for x_index in range(1, Num_of_x_cell-1):
    for y_index in range(0, Num_of_y_cell-1):
      ...
      r = np.sqrt(x**2+y**2)
      if (np.any(np.all(unit_circle_list == np.array([x, y]), axis=1))):
        # The point is on the unit circle, convection needed.
        qx = - lambda_heat * (T[x_index, y_index] - T[x_index-1, y_index])/cell_size_x
        qy = 0
        if (y_index>0):
          qy = - lambda_heat * (T[x_index, y_index] - T[x_index, y_index-1])/cell_size_y
        dT_dt=(qx/cell_size_x+qy/cell_size_y-h*arc_length*(T[x_index, y_index]-T0)/(cell_size_x*cell_size_y))/pho_cp
        T_new[x_index, y_index]+=time_step*(dT_dt)
        print(f'On the edge x={x}, y={y}, r={r}. T was {T[x_index, y_index]}, dT_dt={dT_dt}, T is now {T_new[x_index, y_index]}, qconv={-h*arc_length*(T[x_index, y_index]-T0)}, qconv/qx={(h*arc_length*(T[x_index, y_index]-T0)/(cell_size_x*cell_size_y))/(qx/cell_size_x)}')
      elif (r>=1):
